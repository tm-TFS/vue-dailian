...
window.LArea = (function() {
	...
	MobileArea.prototype = {
		init: function(params) {
			...
			this.level = params.level || 3; //菜单级数
			switch(this.type) {
				case 1:
				case 2:
					break;
		...
		bindEvent: function() {
			...
			function popupArea(e) {
				...
				var _html = '<div class="area_ctrl slideInUp">' +
					...
					'</div>' +
					'</div>';
				if((_self.level == 2) || (_self.level == 3)) {
					_html += '<div>' +
		...
		finish: function(e) {
			...
			var cityVal, cityText, cityCode, countyVal, countyText, countyCode;
			if(area_city) {
			...
			if(this.valueTo) {
				this.valueTo = provinceCode + ((cityCode) ? (',' + cityCode) : ('')) + ((countyCode) ? (',' + countyCode) : (''));
